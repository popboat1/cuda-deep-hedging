...
__global__ void bptt_backward(
    ...
    for(int t = params.num_steps - 1; t >= 0; --t){
        ...
        // layer 3 grad
        atomicAdd(&grads.d_gb3[0], d3);
        for(int j = 0; j < hidden_dim; ++j){
            atomicAdd(&grads.d_gW3[j], d3 * h2[j]);
        }

        // layer 2 grad
        for(int i = 0; i < hidden_dim; ++i){
            atomicAdd(&grads.d_gb2[i], d2[i]);
            for(int j = 0; j < hidden_dim; ++j){
                atomicAdd(&grads.d_gW2[i * hidden_dim + j], d2[i] * h1[j]);
            }
        }

        // layer 1 grad
        for (int i = 0; i < hidden_dim; ++i) {
            atomicAdd(&grads.d_gb1[i], d1[i]);
            for (int j = 0; j < input_dim; ++j) {
                atomicAdd(&grads.d_gW1[i * input_dim + j], d1[i] * x[j]);
            }
        }
    }
}
